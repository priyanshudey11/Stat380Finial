...
calculate_win_stats <- function(data) {
    # Combine Map1 and Map2 into a candidate map column
    all_maps <- unique(c(data$Map1, data$Map2))
    all_maps <- all_maps[all_maps != "" & !is.na(all_maps)]  # Remove blanks and NAs
    
    # Calculate map appearances and wins
    results <- data.frame(Map = all_maps)
    results <- results %>%
        rowwise() %>%
        mutate(
            Appearances = sum(data$Map1 == Map | data$Map2 == Map, na.rm = TRUE),
            # Regular wins: Choice matches the map and it's not a tie (Map1 != Map2)
            Regular_Wins = sum(data$Choice == Map & !(data$Map1 == Map & data$Map2 == Map), na.rm = TRUE),
            # Tie wins: Choice matches the map and it's a tie (Map1 == Map2)
            Tie_Wins = sum(data$Choice == Map & data$Map1 == Map & data$Map2 == Map, na.rm = TRUE),
            Total_Wins = Regular_Wins + Tie_Wins,
            Win_Rate = ifelse(Appearances > 0, round((Total_Wins / Appearances) * 100, 2), 0)
        ) %>%
        ungroup()
    
    # Ensure Win Rate is capped at 100%
    results$Win_Rate <- pmin(results$Win_Rate, 100)
    return(results)
}
...
maxresults <- maxresults %>%
    filter(Map != "" & !is.na(Map))
maxresults <- maxresults %>%
    mutate(Win_Rate = ifelse(Win_Rate > 100, 100, Win_Rate))
maxresults <- maxresults %>%
    mutate(Win_Rate = ifelse(Win_Rate > 100, 100, Win_Rate))
